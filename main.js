...
function popup() {
    ...
    setTimeout(function () { document.querySelector(".popup").remove() }, 4000)
}
...
for (let i = 0; i < color.length; i++) {
    tickbox[i].addEventListener("click", function () {
        if (color[i].style.backgroundColor == "") { color[i].style.backgroundColor = "#F8EF00" }
        else { color[i].style.backgroundColor = "" }
        if (color[0].style.backgroundColor == "" && color[1].style.backgroundColor == "" && color[2].style.backgroundColor == "" && color[3].style.backgroundColor == "") {
            range.value = "1"
            positionchange()
            position.innerHTML = ""
        }
...
function generate() {
    ...
    for (let i = 0; i < color.length; i++) {
        if (color[i].style.backgroundColor != "") {
            colorcounter++
            if (colorcounter >= 1) { passcheck[3].style.backgroundColor = "red" }
            if (colorcounter >= 2) {
...
function date() {
    time = new Date()
    fulldate = `${time.getDate()}/${time.getMonth() + 1}/${time.getFullYear()}  ${time.getHours()}:${time.getMinutes() < 10 ? "0" : ""}${time.getMinutes()}:${time.getSeconds() < 10 ? "0" : ""}${time.getSeconds()}`
}
//create history
function show() {
    date()
...
<div class="last">
    ...
    if (document.getElementsByClassName("last").length == 71) { document.getElementsByClassName("last")[0].remove() }
...
function rangechange() {
    ...
    positionchange()
    //none selected
    if (color[0].style.backgroundColor == "" && color[1].style.backgroundColor == "" && color[2].style.backgroundColor == "" && color[3].style.backgroundColor == "") { position.innerHTML = "" }
    //else
    else {
